# Canada stock market API

Real-time Canada stock market API with WebSocket, OHLCV, and snapshot endpoints for TSX, NEO, and CSE.

## Overview

Kun Data provides real-time market data for Canada through a token-authenticated API stack designed for charting tools, watchlists, market overview pages, brokerage dashboards, and quant workflows.

This repository focuses on Canada market coverage with support for NEO, TSX, CSE.

## API Capabilities

- Real-time WebSocket market data for Canada
...
`Canada stock market API | TSX API | NEO API | Canadian stock data API`

This repository is designed to make Canada market API coverage easier to discover and evaluate.
